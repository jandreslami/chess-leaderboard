...
import { TIPOS_LEADERBOARD, BANDERAS_EXOTICAS, TITULOS } from './diccionarios.js';
...
function crearFilaLeaderboard(jugadorEnLeaderboard) {
  ...
  const partidasTotales = partidasGanadas + partidasPerdidas + partidasTablas;
  ...
  $porcentajeGanadas.textContent = `${(partidasGanadas / partidasTotales * 100).toFixed(2)}%`;
...
async function crearLeaderboard(leaderboard, funcionCallback) {
  leaderboard.rankingJugadoresEnLeaderboard.forEach(jugadorEnLeaderboard => {
...
async function crearTiposLeaderboard(leaderboardSeleccionadoPorDefecto, funcionCallback) {
...
export { limpiarPerfil, mostrarCargando, ocultarCargando, popularPerfil, crearLeaderboard, crearTiposLeaderboard, obtenerJugadorSeleccionado, mostrarPlaceholders };